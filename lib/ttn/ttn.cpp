...
#include <SPI.h>
// clang-format off
// Note : this is a workaround for a bug in the arduino-lmic library
#include <lmic.h>
#include <hal/hal.h>
// clang-format on
        ...
        case EV_JOINED: {
            ...
            if (!LORAWAN_ADR) {
                LMIC_setLinkCheckMode(
                    0);  // Link check problematic if not using ADR. Must be set after join
            }
...
void ttn_register(void (*callback)(uint8_t message)) { _lmic_callbacks.push_back(callback); }
...
void ttn_join() {
    ...
    forceTxSingleChannelDr();
#else
    // Set default rate and transmit power for uplink (note: txpow seems to be ignored by the
    // library)
    ttn_sf(LORAWAN_SF);
#endif
...
void ttn_sf(unsigned char sf) { LMIC_setDrTxpow(sf, 14); }
...
uint32_t ttn_get_count() { return count; }

static void ttn_set_cnt() {
    LMIC_setSeqnoUp(count);

    // We occasionally mirror our count to flash, to ensure that if we lose power we will at least
    // start with a count that is almost correct (otherwise the TNN network will discard packets
    // until count once again reaches the value they've seen).  We limit these writes to a max rate
    // of one write every 5 minutes.  Which should let the FLASH last for 300 years (given the ESP32
    // NVS algoritm)
    static uint32_t lastWriteMsec = UINT32_MAX;  // Ensure we write at least once
    uint32_t now                  = millis();
    if (now < lastWriteMsec ||
        (now - lastWriteMsec) > 5 * 60 * 1000L) {  // write if we roll over (50 days) or 5 mins
        lastWriteMsec = now;
