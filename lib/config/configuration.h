...
#define LORAWAN_CONFIRMED_EVERY \
    0  // Send confirmed message every these many messages (0 means never)
#define LORAWAN_SF \
    DR_SF7  // Spreading factor (recommended DR_SF7 for ttn network map purposes, DR_SF10 works for
            // slow moving trackers)
#define LORAWAN_ADR 0       // Enable ADR
#define REQUIRE_RADIO true  // If true, we will fail to start if the radio is not found

// If not defined, we will wait for lock forever
#define GPS_WAIT_FOR_LOCK \
    (60 * 1000)  // Wait after every boot for GPS lock (may need longer than 5s because we turned
                 // the gps off during deep sleep)
